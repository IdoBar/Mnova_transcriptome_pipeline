...
output: 
    bookdown::html_document2:
      ...
      theme: 
        version: 5
        bootswatch: simplex #sandstone #zephyr # yeti
        # primary: "#6CC3D4"
      highlight: pygments
      css: "style/style.css"
      toc: true
...
Note that we needed to change the `ORP_TRANSRATE` module to use the [ORP Docker container](https://hub.docker.com/r/macmaneslab/orp){target="_blank"} and to make sure it activates the internal `orp` conda environment within the container and disable binding the `/home` folder.
...
Or with the [Oyster River Protocol](https://oyster-river-protocol.readthedocs.io/en/latest/index.html){target="_blank"} pipeline [@macmanesOysterRiverProtocol2018a].
