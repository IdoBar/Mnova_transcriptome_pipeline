...
The reads were aligned to the reference genome with `HISAT` [@kimGraphbasedGenomeAlignment2019], followed by the reconstruction of transcripts with `StringTie` (or [`PsiCLASS`](https://github.com/splicebox/PsiCLASS)). Transcript and gene counts were generated from the alignments using `featurecounts` [@liaoFeatureCountsEfficientGeneral2014], [`HTSeq-count`](http://htseq.readthedocs.io/en/release_0.9.1/count.html) [@andersHTSeqPythonFramework2015] or `Ballgown`, as detailed in @perteaTranscriptlevelExpressionAnalysis2016. 
...
The assembled genome-fuided transcriptome was annotated using BLASTn v2.16.0 [@camachoBLASTArchitectureApplications2009] against the non-redundant nucleotide database of the NCBI (`nt`) to achieve more accurate species-specific annotations. Considering that we're dealing with a plant transcriptome (hopefully similar to well-annotated plant species), it may also be useful to annotate the transcripts against the `refseq_rna` database (which only contains curated gene transcripts).  
BLASTn was run with [blast-nf](https://github.com/IdoBar/nf-blast.git) v0.4.2, a Nextflow pipeline that uses a "split-combine" approach to split the input query (entire transcriptome or proteome) to smaller "chunks" that are run in parallel on the HPC cluster. 
